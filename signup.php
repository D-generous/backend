...
$query = "SELECT statecode FROM `acceptedvoter` WHERE statecode='$statecode'";
$con = $database_con->query($query);

if ($con->num_rows > 0) {

    $query2 = "SELECT * FROM `voters` WHERE statecode='$statecode'";
    ...
    if ($connn->num_rows > 0) {
        ...
        $query3 = "SELECT * FROM `voters` WHERE email='$email'";
        ...
        if ($connnn->num_rows>0) {
            ...
            $insertquery = "INSERT INTO `voters` (`fullname`, `statecode`, `email`, `password`) VALUES ('$name', '$statecode', '$email', '$hashedpass')";
    ...
    echo json_encode("Mr man I don't recognize you. You do not belong to SDG's CDS!!!");
